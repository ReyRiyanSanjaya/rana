...
class _WalletScreenState extends State<WalletScreen> with SingleTickerProviderStateMixin {
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                   ...
                   // 2. Quick Actions
                   Text('Menu Cepat', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF334155))),
                   ...
                   // 3. Pending
                   if (provider.pendingTopUps.isNotEmpty || provider.pendingWithdrawals.isNotEmpty) ...[
                   ...
                   Row(
                     ...
                     children: [
                       Text('Riwayat Transaksi', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF334155))),
           ...
           Row(
             ...
             children: [
               Column(
                 crossAxisAlignment: CrossAxisAlignment.start,
                 children: [
                   Text('Total Saldo', style: GoogleFonts.outfit(color: Colors.white70, fontSize: 13, fontWeight: FontWeight.w500)),
                   const SizedBox(height: 4),
                   Text(
                     _currency.format(provider.balance),
                     style: GoogleFonts.outfit(color: Colors.white, fontSize: 30, fontWeight: FontWeight.bold, letterSpacing: -0.5),
                   ),
                 ],
               ),
               // Logo or Icon
             ...
             children: [
                Row(
                  children: [
                    ...
                    Text('Merchant Pro', style: GoogleFonts.outfit(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
                  ],
                ),
                Container(
  ...
  Widget _buildQuickActions(BuildContext context) {
    ...
    return Wrap(
      ...
      children: actions.map((a) {
         return SizedBox(
           width: (MediaQuery.of(context).size.width - 48 - 60) / 4, // 4 cols approx
           child: Column(
             children: [
               Material(
                 color: Colors.white,
         ...
         SizedBox(
           ...
           child: TabBarView(
             ...
             children: [
               ...
               _buildHistoryList(provider.history.where((i) => i['type'] == 'CASH_IN').toList()),
               _buildHistoryList(provider.history.where((i) => i['type'] == 'CASH_OUT').toList()),
             ],
           ),
         )
       ],
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('Tidak ada riwayat', style: GoogleFonts.outfit(color: Colors.grey[400])),
          ],
        ),
      );
    }
             ...
             children: [
               ...
               Expanded(
                 child: Column(
                   crossAxisAlignment: CrossAxisAlignment.start,
                   children: [
                     Text(item['description'] ?? item['category'] ?? 'Transaksi', style: GoogleFonts.outfit(fontWeight: FontWeight.bold, fontSize: 14)),
                     const SizedBox(height: 4),
                     Text(DateFormat('dd MMM HH:mm').format(DateTime.parse(item['occurredAt'])), style: GoogleFonts.outfit(fontSize: 11, color: Colors.grey[500])),
                   ],
                 ),
               ),
               Text(
                 '${isIncome ? '+' : '-'}${_currency.format(double.parse(item['amount'].toString()))}',
                 style: GoogleFonts.outfit(fontWeight: FontWeight.bold, color: color, fontSize: 14),
               )
             ],
           ),
  ...
  Widget _buildPendingSection(WalletProvider provider) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Menunggu Persetujuan', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF334155))),
        const SizedBox(height: 12),
        ...provider.pendingTopUps.map((e) => _buildPendingCard(e, 'Top Up', Colors.blue)),
        ...provider.pendingWithdrawals.map((e) => _buildPendingCard(e, 'Penarikan', Colors.orange)),
      ],
    );
  }
  
  Widget _buildPendingCard(Map<String, dynamic> item, String type, Color color) {
    return Container(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('$type Pending', style: GoogleFonts.outfit(fontWeight: FontWeight.bold, color: color)),
                Text(_currency.format(item['amount']), style: GoogleFonts.outfit(fontWeight: FontWeight.bold)),
              ],
            ),
          ),
          Text(DateFormat('dd MMM').format(DateTime.parse(item['createdAt'])), style: GoogleFonts.outfit(fontSize: 12, color: Colors.grey)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Top Up Saldo', style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          Container(
            padding: const EdgeInsets.all(16),
               ...
               children: [
                 ...
                 Expanded(
                   child: Column(
                     crossAxisAlignment: CrossAxisAlignment.start,
                     children: [
                       Text('BCA Virtual Account', style: GoogleFonts.outfit(color: const Color(0xFF881337), fontWeight: FontWeight.bold)),
                       Text('88012 081234567890', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.w600, letterSpacing: 1)),
                     ],
                   ),
                 ),
          ...
          const SizedBox(height: 24),
          TextField(controller: _amountCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Nominal Transfer', prefixText: 'Rp ')),
          const SizedBox(height: 16),
          Text('Bukti Transfer (Wajib)', style: GoogleFonts.outfit(fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          InkWell(
            onTap: _pickImage,
            child: Container(
              height: 150,
              decoration: BoxDecoration(border: Border.all(color: Colors.grey[300]!, width: 2), borderRadius: BorderRadius.circular(16), color: Colors.grey[50]),
    ...
    Widget build(BuildContext context) {
      return Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
             Text('Transfer ke Merchant Lain', style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold)),
             const SizedBox(height: 24),
             TextField(controller: _storeIdCtrl, decoration: const InputDecoration(labelText: 'Store ID Penerima')),
             const SizedBox(height: 16),
             TextField(controller: _amountCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Nominal Transfer', prefixText: 'Rp ')),
             const SizedBox(height: 16),
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
            Text('Tarik Saldo', style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 24),
            TextField(controller: _amountCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Nominal Penarikan', prefixText: 'Rp ')),
            const SizedBox(height: 16),
            TextField(controller: _bankCtrl, decoration: const InputDecoration(labelText: 'Nama Bank')),
            const SizedBox(height: 16),
